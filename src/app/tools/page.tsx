  ...
  const renderTool = () => {
    ...
    return (
      <div style={{background:'white',borderRadius:'24px',padding:'32px',boxShadow:'0 10px 40px rgba(0,0,0,0.2)'}}>
        ...
        {activeTool==='music' && (
          <div>
            <audio ref={audioRef} src={currentSong?.url || ''} onTimeUpdate={() => setProgress(audioRef.current?.currentTime || 0)} onLoadedMetadata={() => setDuration(audioRef.current?.duration || 0)} onEnded={nextSong} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} />
            ...
            <div style={{padding:'32px',background:'linear-gradient(135deg,#667eea 0%,#764ba2 100%)',borderRadius:'16px',color:'white',marginBottom:'24px',textAlign:'center'}}>
              {currentSong ? (
                ...
                <div style={{padding:'40px'}}><p style={{fontSize:'48px',marginBottom:'16px'}}>🎵</p><p>Select or import a song to start!</p></div>
              )}
            </div>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px',maxHeight:'400px',overflowY:'auto'}}>
              {playlist.map(song=>(
                <div key={song.id} onClick={()=>playSong(song)} style={{padding:'16px',background:currentSong?.id===song.id?'#f3f4f6':'white',borderRadius:'12px',border:`2px solid ${currentSong?.id===song.id?'#667eea':'#e5e7eb'}`,cursor:'pointer',display:'flex',alignItems:'center',gap:'16px'}}>
                  ...
                  <div style={{flex:1}}><p style={{fontWeight:'bold',margin:0}}>{song.title}</p><p style={{margin:'4px 0 0',color:'#6b7280',fontSize:'12px'}}>{song.isDefault ? 'Built-in' : 'Imported'}</p></div>
